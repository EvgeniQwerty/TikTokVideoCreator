...
def downloadMusicFromPlaylist(playlist_url):
        ...
        try:
            ...
            if len(title) == 2:
                if title[0].find(',') >= 0:
                    ...
                    txt = concatTxt(title[0], title[1], 'feat')
                elif title[0].find(' x ') >= 0:
                    txt = concatTxt(title[0], title[1], ' x ')
                else:
                    txt = concatTxt(title[0], title[1])
            else:
...
def createParser():
    ...
    parser.add_argument('-dv', '--downloadVisuals', default='false')
    parser.add_argument('-vpu', '--visualsPlaylistURL',
                        default='https://www.youtube.com/playlist?list=PLCqJefJxoW8wRGX7iAgImBcz084j9wM0h')
    parser.add_argument('-dm', '--downloadMusic', default='true')
    parser.add_argument('-mpu', '--musicPlaylistURL',
                        default='https://www.youtube.com/playlist?list=PLUGshIgZ8go9HT0dIUqnw3T2TjVB69ba8')
...
if __name__ == '__main__':
    ...
    if namespace.downloadVisuals == 'true':
        downloadVideosFromPlaylist(namespace.visualsPlaylistURL)

    if namespace.downloadMusic == 'true':
        downloadMusicFromPlaylist(namespace.musicPlaylistURL)
